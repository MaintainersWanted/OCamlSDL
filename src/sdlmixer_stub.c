...
/* $Id: sdlmixer_stub.c,v 1.1 2000/01/31 20:09:06 smkl Exp $ */
...
{
   int ret;
   ret = Mix_AllocateChannels(Int_val(channels));
   return Val_int(ret);
}
...
{
   if (music_finished != Val_unit) callback(music_finished, Val_unit);
}
...
{
   int res;
   res = Mix_ReserveChannels(Int_val(num));
   return Val_int(res);
}
...
sdlmixer_group_channel(value chn, value grp)
{
   ...
   if (!res) sdlmixer_raise_exception(Mix_GetError());
   return Val_unit;
}
...
{
   int res;
   res = Mix_GroupAvailable(Int_val(grp));
   return Val_int(res);
}
...
{
   int res;
   res = Mix_GroupCount(Int_val(grp));
   return Val_int(res);
}
...
{
   int res;
   res = Mix_GroupOldest(Int_val(grp));
   return Val_int(res);
}
...
{
   int res;
   res = Mix_GroupNewer(Int_val(grp));
   return Val_int(res);
}
...
{
   int res;
   res = Mix_PlayChannelTimed(MAGIC_OF_OPTION(chn), (Mix_Chunk *)sound,
...
{
   int res;
   res = Mix_PlayMusic((Mix_Music *)music, MAGIC_OF_OPTION(loops));
   return Int_val(res);
}
...
{
   int res;
   res = Mix_FadeInMusic((Mix_Music *)music, MAGIC_OF_OPTION(loops),
			 MAGIC_OF_OPTION(tme));
   return Int_val(res);
}
...
{
   int res;
   res = Mix_FadeInChannelTimed(MAGIC_OF_OPTION(chn), (Mix_Chunk *)chunk,
				MAGIC_OF_OPTION(loops), MAGIC_OF_OPTION(tme),
...
{
   int res;
   res = Mix_Volume(MAGIC_OF_OPTION(chn), MAGIC_OF_OPTION(vol));
   return Int_val(res);
}
...
{
   int res;
   res = Mix_VolumeChunk((Mix_Chunk *)chunk, MAGIC_OF_OPTION(vol));
   return Int_val(res);
}
...
{
   int res;
   res = Mix_VolumeMusic(MAGIC_OF_OPTION(vol));
   return Int_val(res);
}
...
convert_fading_status(Mix_Fading f)
{
   ...
   switch (f)
     {
      case MIX_NO_FADING: conv = 0; break;
      case MIX_FADING_OUT: conv = 1; break;
      case MIX_FADING_IN: conv = 2; break;
     }
   return Int_val(conv);
}
...
{
   int res;
   res = Mix_Paused(Int_val(chn));
   return res ? Val_int(1) : Val_int(0);
}
...
{
   int res;
   res = Mix_Playing(MAGIC_OF_OPTION(chn));
   return res ? Val_int(1) : Val_int(0);
}
